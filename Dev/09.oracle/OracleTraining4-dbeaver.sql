...
-- 6-2. 대소문자 구분없이 검색하기
-- 사용자가 'scott', 'SCOTT', 'Scott' 등 어떻게 입력해도 찾을 수 있음
SELECT *
FROM EMP
...
FROM EMP
WHERE LENGTH(ENAME) > 5;	-- 글자 수가 5 이상인 경우만 
...
SELECT JOB,
	...
	SUBSTR(JOB, -LENGTH(JOB), 2), 	-- 맨 뒤에서 전체 길이 위치부터 2글자 자르기
	SUBSTR(JOB, -3)					-- 맨 뒤에서 3번째부터 끝까지 자르기
FROM EMP;
...
-- 6-9 : INSTR() - 문자 위치 찾기
-- 특정 문자가 처음 나타나는 위치를 반환 (없으면 0 반환)
-- INSTR(문자열, 찾을 문자) : 기본 형태 (첫 번째 위치 반환)
-- INSTR(문자열, 찾을 문자, 시작 위치) : 특정 위치부터 검색
-- INSTR(문자열, 찾을 문자, 시작위치, N번째) : 문자가 N번째 출현하는 위치
-- INSTR(문자열, 찾을 문자, -1) : 뒤에서부터 검색
SELECT INSTR('HELLO, ORACLE!', 'L') AS INSTR_1, 		-- 첫 번째 L의 위치
		INSTR('HELLO, ORACLE!', 'L', 5) AS INSTR_2,		-- 5번째 이후 첫 L의 위치
		INSTR('HELLO, ORACLE!', 'L', 2, 2) AS INSTR_3	-- 2번째부터 시작해서 2번째 L의 위치
FROM DUAL;

-- 6-10 : INSTR을 WHERE 절에서 사용
...
SELECT LOWER(ENAME) || '@gmail.com' AS "이메일",	-- 이메일 생성
	...
	SUBSTR(LOWER(ENAME) || '@gmail.com', 1, 
			INSTR(LOWER(ENAME) || '@gmail.com', '@') -1) AS "아이디",
	SUBSTR(LOWER(ENAME) || '@gmail.com', 
			INSTR(LOWER(ENAME) || '@gamil.com', '@') +1) AS "도메인" -- @뒷 부분 추출
FROM EMP
WHERE ROWNUM <= 5;

-- 6-12 : REPLACE() - 문자 지환
-- REPLACE(원본, 찾을문자, 바꿀문자)
-- 세번째 매개변수 생략 시 해당 문자 제거 
SELECT '010-1234-5678' AS REPLACE_BEFOR,
		REPLACE('010-1234-5678', '-', ' ') AS "-를 공백으로 치환",	-- -를 공백으로 치환
		REPLACE('010-1234-5678', '-') AS "-를 제거"				-- -를 제거
FROM DUAL;
...
SELECT 
	...
	REPLACE(REPLACE(REPLACE(ENAME, 'S', '*'), 'A', '#'), 'E', '@') AS "3단계_E"
FROM EMP
...
SELECT '1234-5678-9012-3456' AS "원본카드번호",
	...
	RPAD('*', 4, '*') || '-' ||
	RPAD('*', 4, '*') || '-' ||
	SUBSTR('1234-5678-9012-3456', -4) AS "마스킹 결과"
FROM DUAL; 

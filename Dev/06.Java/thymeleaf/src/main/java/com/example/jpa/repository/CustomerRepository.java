...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    // 1) JpaRepository가 제공하는 기본 메서드들 : save(), findById(), deleteById, count()..
    ...
    /*
        Like 검색 - Containing (포함)
            - 특정 문자가 포함된 고객 검색
            - 생성된 SQL: SELECT * FROM username Like %?%
     */
    List<Customer> findByCustomerNameContaining(String containingName);
    ...
    /*
        Like 검색 - StartingWith (시작)
            ...
            - 생성된 SQL: SELECT * FROM username Like ?%%
     */
    List<Customer> findByUsernameStartingWith(String freFix);

    /*
        정렬 - Order By
            - 등급별로 나이 내림차순 정렬
            - 생성된 SQL: SELECT * FROM rating = ? ORDER BY age DESC
     */
    ...
    List<Customer> findByAgeBetweenAndRatingOrderByReservesDesc(
            int age, int endAge, String rating
    );

    /*
        문제 : 삭제되지 않은 활성 회원들의 정보를 조회하시오.
     */
    ...
    // =================== JPQL =====================
    /*
        문제 : 입력받은 등급(rating)과 일치하면서, 삭제 상태(deleted)가 false인 회원들을 모두 조회하는 JPQL을 작성하시오
     */
    ...
    List<Customer> findPlatinumCustomerInAgeRange(@Param("minAge") int minAge,
                                                  @Param("maxAge") int maxAge);

    /*
        문제 3 : 삭제되지 않은 활성 회원들을 등급 별로 그룹화하여 다음 통계 정보를 출력하시오.
        출력 - 등급명, 회원 수, 평균 나이, 총 적립금 합계 출력
              회원 수가 많은 등급부터 정렬하여 출력하시오
     */

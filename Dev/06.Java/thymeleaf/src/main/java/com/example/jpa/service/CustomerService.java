...
public class CustomerService {
    ...
    public void test3FindByCustomerNameOrOccupation() {
        ...
        customerRepository.findByCustomerNameOrOccupation("손홍민", "개발자");
        System.out.println("* 검색 조건 : 이름='손홍민' OR '개발자'");
    ...
    public void test4FindByAgeGreaterThan() {
        System.out.println("\n [test3] test4FindByAgeGreaterThan");
        System.out.println("==========================================");
    ...
    public void test5FindByAgeLessThanEqual() {
        ...
        List<Customer> underAge = customerRepository.findByAgeLessThanEqual(40);
        System.out.println("* 검색 조건 : 나이 <= 40 ");
        System.out.println("* 검색 결과 : "+ underAge.size() +"명");
        underAge.stream().limit(5).forEach(c ->
                System.out.println(" => " +c.getCustomerName()+ "(" +c.getAge()+ "세)"));
        System.out.println("\n 생성된 SQL: SELECT * FROM customer WHERE age <= ?");
    }
    ...
    public void test7FindByCustomerNameContaining() {
        ...
        List<Customer> nameContains = customerRepository.findByCustomerNameContaining("이");
        System.out.println("* 검색 조건 : 이름에 '이' 포함");
        System.out.println("* 검색 결과 : "+ nameContains.size() +"명");
        nameContains.forEach(c ->
    ...
    public void test8FindByUsernameStartingWith() {
        ...
        List<Customer> startsWith = customerRepository.findByUsernameStartingWith("user");
        System.out.println("* 검색 조건 : username이 'user'로 시작");
    ...
    public void test9FindByRatingOrderByAgeDesc() {
        ...
        System.out.println("* 검색 조건 : PLATINUM 등급, 나이 내림차순");
    ...
    public void test10FindByAgeBetweenAndRatingOrderByReservesDesc() {
        ...
        List<Customer> complex = customerRepository.findByAgeBetweenAndRatingOrderByReservesDesc(20, 30, "PLATINUM");
        System.out.println("* 검색 조건 : 20-30세 AND PLATINUM 등급, 적립금 내림차순");
        System.out.println("* 검색 결과 : "+ complex.size() +"명");
        complex.forEach(c ->
                System.out.println(" => " + c.getCustomerName() + "(적립금 : "+ c.getReserves() + ")"));
        System.out.println("\n 생성된 SQL: SELECT * FROM customer WHERE rating = ? ORDER BY reserves DESC");
    }
    ...
    public void test11FindByDeletedFalse() {
        ...
        System.out.println("* 활성 고객 수 : "+ activeCustomers.size() +"명");
    ...
    public void test13ExistsByUsername() {
        ...
        boolean exists1 = customerRepository.existsByUsername("campusseven24");
        boolean exists2 = customerRepository.existsByUsername("springai");

        System.out.println(" * 'campusseven24' 존재 여부:  " + (exists1 ? "✅ 존재" : "❌ 없음"));
        System.out.println(" * 'springai' 존재 여부:  " + (exists2 ? "✅ 존재" : "❌ 없음"));
    ...
    public void jpqlTest1FindActiveCustomersByRating() {
        ...
        List<Customer> silverActive =
                customerRepository.findActiveCustomersByRating("GOLD");

        System.out.println("* 검색 결과 : " +silverActive.size()+ "명");
        silverActive.stream().limit(3).forEach(c -> {
    ...
    @Transactional(readOnly = true)
    public void jpqlTest2FindPlatinumCustomersInAgeRange() {
        System.out.println("\n [JPQL test2] jpqlTest2FindPlatinumCustomersInAgeRange");
        System.out.println("==========================================");

        List<Customer> platinumInRange =
        customerRepository.findPlatinumCustomersInAgeRange(20, 40);

        System.out.println("* 검색 결과 : " +platinumInRange.size()+ "명");
        platinumInRange.stream().limit(3).forEach(c -> {
            System.out.println(" => " +c.getCustomerName() + " ("
                    + c.getAge() + "세, " + c.getRating() + ")");
        });
    }
    ...
    public void jpqlTest3GetCustomerStaticsByRating() {
        List<Object[]> statics =
        customerRepository.getCustomerStaticsByRating();
        ...
        for (Object[] stat : statics) {
            ...
            Long totalReserves = stat[3] !=null ? (Long) stat[3] : 0L;

            System.out.printf(" => %s등급: %d명, 평균나이: %.1f세, 총적립금: %d원\n",
    ...
    public void jpqlTest4FindCustomersAboveAverageReserves() {
        List<Customer> aboveAverages =
        customerRepository.findCustomersAboveAverageReserves();

        System.out.println(" * 검색 결과 : " + aboveAverages.size() + "명");
        aboveAverages.stream().limit(3).forEach(c -> {
            System.out.println(" => " + c.getCustomerName() +
                    " (적립금 : " + c.getReserves() + ")");
        });
        System.out.println("\n 특징 : where 절에 서브쿼리 사용");

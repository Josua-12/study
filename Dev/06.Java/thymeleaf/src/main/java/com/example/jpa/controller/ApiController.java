...
@Slf4j                      // Lombok 로킹 기능 자동 추가 (log 변수 사용 가능)
@RestController             // @Controller + @ResponseBody => 모든 메서드가 JSON 반환
@RequestMapping("/api")        // 기본 경로 매핑, 애플리케이션 루트 url  /api로 시작
public class ApiController {
    ...
    public ApiController() {
        ...
        messages.add(MessageDTO.builder()
                .id(idCounter++)    // ID : 2
                .title("두 번째 메시지")
                .content("비동기 요청 처리에 오신 것을 환영합니다!")
                .author("Admin")
                .createAt(LocalDateTime.now().minusMinutes(5))  // 5분전
                .success(true)
                .build()
        );
        ...
        messages.add(MessageDTO.builder()
                ...
                .author("Developer")
                .createAt(LocalDateTime.now().minusMinutes(10))  // 10분전
                .success(true)
                .build()
        );
    }
    ...
    public ResponseEntity<MessageDTO> getMessage(@PathVariable("id") Long id) {
        log.info("GET /api/message/{} - 특정 메시지 조회", id);
